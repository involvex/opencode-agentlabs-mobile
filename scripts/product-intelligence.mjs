...
export function workflowFailures(runs, now, branch) {
  const recentRuns = recent(runs, "created_at", now - WEEK).filter((run) => {
    ...
    return (
      run.conclusion &&
      selectedBranch &&
      path !== SELF_WORKFLOW_PATH &&
      name !== SELF_WORKFLOW_NAME
    );
  });
...
function args(values) {
  ...
  for (let index = 0; index < values.length; index += 2) {
    ...
    if (!key?.startsWith("--") || !value)
      throw new Error(`Expected --name value, received ${key ?? "<none>"}`);
...
function isSentryProvisioningGap(sentry) {
  ...
  return (
    /is not set/.test(sentry.reason) ||
    /HTTP 401/.test(sentry.reason) ||
    /HTTP 403/.test(sentry.reason)
  );
}
...
export async function collectGithub(token, repo, now) {
  ...
  const [repository, releases, issues, workflows, views, clones] =
    await Promise.all([
...
export async function collectSentry(token, organization, project, now) {
  if (!token || !organization || !project) {
    return unavailable(
      "SENTRY_AUTH_TOKEN, SENTRY_ORG, or SENTRY_PROJECT is not set",
    );
  }
  const url = new URL(
    `${SENTRY_API}/projects/${organization}/${project}/issues/`,
  );
...
function sourceLine(name, source, degraded) {
  if (source.status === "available")
    return `| ${name} | available | current run |`;
  const detail = degraded
    ? `token not provisioned — ${source.reason}`
    : source.reason;
...
function render(report) {
  const github =
    report.github.status === "available" ? report.github.data : null;
  const sentry =
    report.sentry.status === "available" ? report.sentry.data : null;
  const state = report.material
...
async function main() {
  ...
  if (githubData?.activeWorkflowFailureStreaks >= 1)
    signals.push("repeated-workflow-failure");
  ...
  if (process.env.GITHUB_STEP_SUMMARY)
    await write(process.env.GITHUB_STEP_SUMMARY, markdown);
