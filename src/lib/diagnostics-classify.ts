...
export function classify(
  ...
  if (!parsed.valid) {
    return {
      classification: "malformed-url",
      summary:
        "The server URL could not be parsed. Check for typos or extra characters.",
    };
  }
  if (health.ok) {
    return {
      classification: "ok",
      summary: "Health endpoint responded — connection actually works now.",
    };
  }
  ...
  if (isTls) {
    return {
      ...
      summary:
        "TLS/certificate problem. Try http:// instead of https://, or fix the server certificate.",
    };
  }
  ...
  if (!internet.ok) {
    return {
      classification: "no-internet",
      summary:
        "The device has no working internet/network at all (public check also failed). Check Wi-Fi/data and Tailscale (VPN) status.",
    };
  }
  ...
  if (isTimeout) {
    return {
      ...
      summary:
        "Connection to the server timed out (dropped, not refused). Likely a firewall, wrong port, or Tailscale ACL blocking the device.",
    };
  }
  return {
