...
test("rejects malformed or non-http(s) URLs", () => {
  for (const bad of [
    ...
    assert.equal(
      parseUrl(bad).valid,
      false,
      `should be invalid: ${JSON.stringify(bad)}`,
    );
  }
});
...
test("TLS-flavored error wins even when internet is down", () => {
  const r = classify(
    okUrl,
    probe({ error: "SSL handshake failed" }),
    probe({ ok: false }),
    probe(),
  );
...
test("detects TLS keywords from errorCause too", () => {
  const r = classify(
    okUrl,
    probe({ errorCause: "certificate has expired" }),
    probe({ ok: true }),
    probe(),
  );
  assert.equal(r.classification, "tls-error");
...
test("no internet (and no TLS signal) classifies as no-internet", () => {
  const r = classify(
    okUrl,
    probe({ error: "Network request failed" }),
    probe({ ok: false }),
    probe(),
  );
...
test("server-unreachable adds MagicDNS hint only for hostnames, not IPs", () => {
  ...
  const hostR = classify(
    parseUrl("http://box.ts.net:8080"),
    probe({ error: "refused" }),
    fail.internet,
    fail.root,
  );
  const ipR = classify(
    parseUrl("http://10.0.0.5:8080"),
    probe({ error: "refused" }),
    fail.internet,
    fail.root,
  );
