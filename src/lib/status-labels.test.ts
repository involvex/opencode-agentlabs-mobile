...
test("known tools map to their friendly label", () => {
  assert.equal(
    statusFromPart(part({ type: "tool", tool: "grep" })),
    "Searching codebase...",
  );
  assert.equal(
    statusFromPart(part({ type: "tool", tool: "bash" })),
    "Running command...",
  );
  assert.equal(
    statusFromPart(part({ type: "tool", tool: "edit" })),
    "Making edits...",
  );
});
...
test("read/list/grep/glob and edit/write/apply_patch share labels", () => {
  for (const t of ["list", "grep", "glob"]) {
    assert.equal(
      statusFromPart(part({ type: "tool", tool: t })),
      "Searching codebase...",
    );
  }
...
test("an unknown tool degrades to 'Running <tool>...'", () => {
  assert.equal(
    statusFromPart(part({ type: "tool", tool: "frobnicate" })),
    "Running frobnicate...",
  );
});
