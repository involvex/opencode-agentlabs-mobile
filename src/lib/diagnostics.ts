...
export async function probeConnection(
  ...
  if (parsed.valid) {
    ...
    const skipped: ProbeAttempt = {
      name: "health",
      target: url,
      ok: false,
      durationMs: 0,
      error: "skipped: malformed url",
    };
    health = skipped;
  ...
  const report: DiagnosticReport = {
    ...
    device: {
      ...
      appVersion:
        (appJson as { expo?: { version?: string } }).expo?.version || "unknown",
    },
...
export function formatReport(report: DiagnosticReport): string {
  ...
  lines.push(
    `  scheme=${report.scheme} host=${report.host} port=${report.port} hostname=${report.isHostname}`,
  );
  lines.push("");
  lines.push("Probes:");
  for (const a of report.attempts) {
    ...
    lines.push(
      `      ${status} (${a.durationMs}ms)${a.errorCause ? ` cause=${a.errorCause}` : ""}`,
    );
  }
  lines.push("");
  lines.push("Device:");
  lines.push(
    `  ${report.device.platform} ${report.device.osVersion} | ${report.device.model} | app ${report.device.appVersion}`,
  );
  lines.push("");
...
export function buildCrashReport(
  error: unknown,
  source: "react-boundary" | "global" = "global",
): DiagnosticReport {
  const err =
...
export async function shareReport(report: DiagnosticReport): Promise<void> {
  ...
  try {
    await Share.share({
      title: "OpenCode connection diagnostic",
      message: text,
    });
  } catch (e) {
