...
test("loadSessionList: (a) calls /experimental/session first (global)", async () => {
  ...
  assert.ok(
    !t.calls.some((c) => c.startsWith("legacy")),
    "must not hit legacy /session when experimental works",
  );
});
...
test("loadSessionList: (c) falls back to /session on 404 (older server)", async () => {
  ...
  assert.equal(
    t.calls[1],
    "legacy?roots=true&limit=50",
    "must call legacy with preserved query params",
  );
...
test("loadSessionList: non-404 experimental error propagates (no silent fallback)", async () => {
  const t = transport({
    experimentalThrows: new Error("API Error: 500 - boom"),
  });
  await assert.rejects(() => loadSessionList(t, {}), /500/);
  assert.ok(
    !t.calls.some((c) => c.startsWith("legacy")),
    "500 must NOT fall back to legacy",
  );
});
