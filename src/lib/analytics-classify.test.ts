...
test("401 / unauthorized errors -> unauthorized (the known connect bug)", () => {
  assert.equal(
    classifyConnectionError("API Error: 401 - Unauthorized"),
    "unauthorized",
  );
  assert.equal(classifyConnectionError("401"), "unauthorized");
  assert.equal(
    classifyConnectionError("Request failed: unauthorized"),
    "unauthorized",
  );
  assert.equal(
    classifyConnectionError("HTTP 401 Unauthorised"),
    "unauthorized",
  );
});
...
test("TLS / certificate errors -> tls-error", () => {
  ...
  assert.equal(
    classifyConnectionError("certificate verify failed"),
    "tls-error",
  );
...
test("network-level failures -> server-unreachable", () => {
  assert.equal(
    classifyConnectionError("Network request failed"),
    "server-unreachable",
  );
  assert.equal(
    classifyConnectionError("connect ECONNREFUSED 192.0.2.1:4096"),
    "server-unreachable",
  );
  assert.equal(
    classifyConnectionError("host unreachable"),
    "server-unreachable",
  );
...
test("URL parse failures -> malformed-url", () => {
  ...
  assert.equal(
    classifyConnectionError("Invalid URL: htp:/oops"),
    "malformed-url",
  );
});
...
test("precedence: 401 wins over other matches in a combined message", () => {
  ...
  assert.equal(
    classifyConnectionError("401 Unauthorized (tls terminated)"),
    "unauthorized",
  );
});
