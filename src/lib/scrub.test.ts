...
test("redactHostAndUrls: redacts every session host, not just the report's own", () => {
  ...
  const text = [
    '{"host":"box-a.tailnet.ts.net","port":"4096"}',
    "server unreachable box-b.local:8080",
  ].join("\n");
  const out = redactHostAndUrls(text, [
    undefined,
    "box-a.tailnet.ts.net",
    "box-b.local",
  ]);
...
test("redactHostAndUrls: blanks bare IPv4 addresses even when unknown", () => {
  const out = redactHostAndUrls(
    "connect failed 192.168.1.50:4096 via 10.0.0.1",
    [],
  );
