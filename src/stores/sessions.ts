...
} from "../lib/session-load-reconcile";

// Helper to convert API response to our internal format
function parseMessages(response: MessageWithParts[]): {
  messages: Message[];
...
export type RevertResult =
  | ({ ok: true } & PromptFromParts)
  | { ok: false; reason: "unsupported" | "auth" | "error" };
...
export const useSessions = create<SessionsState>((set, get) => ({
    ...
    try {
      ...
      set({
        ...
        parts: {
          ...allParts,
          ...Object.fromEntries(
            temp.map((m) => [m.id, get().parts[m.id] || []]),
          ),
        },
        loadingMore: false,
  ...
  sendMessage: async (text, model, agent, files, variant) => {
    ...
    try {
      set((state) => ({
        sending: { ...state.sending, [session.id]: true },
        error: null,
      }));
      ...
      case "message.removed": {
        ...
        set((state) => ({
          ...
          parts: Object.fromEntries(
            Object.entries(state.parts).filter(([k]) => k !== messageID),
          ),
        }));
        break;
      }
        ...
        set((state) => ({
          ...
          currentSession:
            state.currentSession?.id === session.id
              ? session
              : state.currentSession,
          isLoading: isLiveEventForSession(session.id, state.currentSession?.id)
            ? false
            : state.isLoading,
        }));
        break;
