    ...
    busySessionIDs.map(async (sessionID) => {
      try {
        ...
        const session =
          ...
          (sessionsState.currentSession?.id === sessionID
            ? sessionsState.currentSession
            : undefined);
        const connState = useConnections.getState();
        const client = session?.directory
          ? (connState.clientForDirectory(session.directory) ??
            connState.client)
          : connState.client;
        if (!client) return;
        ...
        // stomp on it.
        if (useEvents.getState().sessionStatus[sessionID]?.type !== "busy")
          return;
        ...
        console.warn(
          "[Events] Failed to resync session status for",
          sessionID,
          err,
        );
      }
  ...
  connect: () => {
    ...
    (async () => {
      ...
      const scheduleReconnect = (reason: unknown) => {
        ...
        console.warn(
          `[SSE] Connection lost, reconnecting in ${jitteredDelay}ms:`,
          reason,
        );
      ...
      try {
        for await (const event of client.global.events(
          ...
          switch (type) {
            case "session.status": {
              ...
              const completed =
                previous?.type === "busy" && status.type === "idle";
            ...
            case "message.updated": {
              ...
              useSessions
                .getState()
                .handleEvent({ type, properties: { info } } as any);
              break;
            }
            ...
            case "message.part.updated": {
              ...
              useSessions
                .getState()
                .handleEvent({ type, properties: { part } } as any);
              break;
            }
            ...
            case "permission.asked": {
              ...
              set((state) => ({
                permissions: {
                  ...state.permissions,
                  [req.sessionID]: [
                    ...(state.permissions[req.sessionID] || []),
                    req,
                  ],
                },
              }));
        ...
        if (isAuthError(err) && !currentController.signal.aborted) {
          ...
          console.warn(
            "[SSE] Authentication failed — stopping reconnect loop:",
            err,
          );
          track(AnalyticsEvent.ConnectionFailed, {
            source: "sse",
