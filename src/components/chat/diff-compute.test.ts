...
test("computeDiff normalizes CRLF vs LF so only the actually-changed line diffs", () => {
  ...
  assert.equal(
    removed.length,
    1,
    "expected exactly one removed line, not a whole-file diff",
  );
  assert.equal(
    added.length,
    1,
    "expected exactly one added line, not a whole-file diff",
  );
...
test("computeDiff falls back to a truncated diff for huge inputs instead of hanging", () => {
  ...
  assert.ok(
    elapsedMs < 2000,
    `computeDiff took ${elapsedMs}ms, expected a fast truncated fallback`,
  );

  // Truncated to at most 400 lines per side plus one synthetic marker line.
  assert.ok(
    result.length <= 801,
    `expected truncated output, got ${result.length} lines`,
  );
